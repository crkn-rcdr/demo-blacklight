...
module Blacklight::Solr
    class Repository < Blacklight::AbstractRepository
      ...
      def search params = {}
        #puts "Orig"
        #puts params.class
        #puts params.blacklight_params

        #if params.blacklight_params[:q].nil? || params.blacklight_params[:q].empty?
        #  puts "Pass"
        #  # {"search_field"=>"title_tsim", "q"=>"British", "controller"=>"catalog", "action"=>"index"}
        #  params.blacklight_params[:q] = "*:*"
        #  #params.delete("q")
        #end
        #puts "After"
        #puts params.blacklight_params
      ...
      def build_solr_request(solr_params)
        if uses_json_query_dsl?(solr_params)
          puts "One"
          solr_params[:json][:query][:bool][:must].delete("")
          pp solr_params
          tcjn = { params: solr_params.to_hash.except(:json) }.merge(solr_params[:json])
          pp tcjn
          {
            data: tcjn.to_json,
            method: :post,
